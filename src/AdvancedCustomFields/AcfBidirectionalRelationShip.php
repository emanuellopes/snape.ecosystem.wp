...
class AcfBidirectionalRelationShip extends AbstractFeature
{
    ...
    public function load(): void
    {
        ...
        add_filter('acf/update_value/type=relationship', array($this, 'updateRelationshipField'), 11, 3);
        add_filter('acf/update_value/type=post_object', array($this, 'updateRelationshipField'), 11, 3);
    }

    public function updateRelationshipField($value, $post_id, $field)
    {
        $update = true;
        ...
        if (count($previous)) {
            foreach ($previous as $related_id) {
                if (! in_array($related_id, $new)) {
                    $this->removeRelationship($related_id, $field_name, $post_id);
        ...
        if (count($new)) {
            foreach ($new as $related_id) {
                $this->addRelationship($related_id, $field_name, $post_id);
                $updated_posts[] = $related_id;
            }
        }
    ...
    private function removeRelationship($post_id, $field_name, $related_id): void
    {
        /*
        ...
        */
        $field = $this->getField($post_id, $field_name);
        if (! $field) {
    ...
    private function addRelationship($post_id, $field_name, $related_id): void
    {
        /*
        ...
        */
        ...
        if (($max_posts == 0 || count($value) < $max_posts) && ! in_array($related_id, $value)) {
            $append_where = 'append';
            $append_where = apply_filters('acf/post2post/append-prepend', $append_where); // all field
            $append_where = apply_filters(
                'acf/post2post/append-prepend/name=' . $field['name'],
                $append_where
            ); // field name
            $append_where = apply_filters(
                'acf/post2post/append-prepend/key=' . $field['key'],
                $append_where
            ); // field key
            ...
            if (isset($overwrite_settings[$field_name]) && isset($overwrite_settings[$field_name]['overwrite'])
                ...
                $this->removeRelationship(intval($remove), $field_name, $post_id);
                $append_where = 'append';
                $append_where = apply_filters('acf/post2post/append-prepend', $append_where); // all field
                $append_where = apply_filters(
                    'acf/post2post/append-prepend/name=' . $field['name'],
    ...
    public function getField($post_id, $field_name)
    {
        $field = false;
        $found = false;
        ...
        $found = false;
        $field_groups = $this->postFieldGroups($post_id);
    ...
    public function postFieldGroups($post_id)
    {
        $found = false;
